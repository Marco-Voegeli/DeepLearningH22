...
from create_demo_img import IMG_PATH, TRAIN_IMG_NAMES_PATH, PYTLSD_PATH
...
def get_thresholded_grad(resized_img):
    ...
    # A B
    # C D
    A, B, C, D = resized_img[:-1, :-1], resized_img[:-
                                                    1, 1:], resized_img[1:, :-1], resized_img[1:, 1:]
...
def analyze_image_pytlsd(image_path='pyt_lsd/resources/ai_001_001.frame.0000.color.jpg'):
    ...
    segments = pytlsd.lsd(
        resized_img, 1.0, gradnorm=gradnorm, gradangle=gradangle)
    segments /= scale_down
    ...
    for segment in segments:
        cv2.line(img_color, (int(segment[0]), int(segment[1])), (int(
            segment[2]), int(segment[3])), (0, 255, 0))
